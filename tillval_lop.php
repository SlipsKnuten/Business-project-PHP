...
echo "<br><br><div id='tillvalDiv'>";
	...
	echo "<div id ='row'>";
		...
		// echo "<button id='valjknapp1' id='valjknapp' type='submit'><img src='pictures/pilHoger.jpg' width='50' height='30'></button>";
		echo "</form>";
	echo "</div>";
...
if(isset($_POST['langning'])){
	...
	$sql = "SELECT pris FROM loplopp WHERE ordningsnr = '$ordningsnr'";
	$result = mysqli_query($dbc, $sql);
	$plopp = mysqli_fetch_array($result);
	
	// echo $plopp;
	
	$items = $pbil[0] + $pdip[0] + $pfor[0] + $plang[0] + $plopp[0] + $ptshirt[0];
	...
	echo $_SESSION['Langning'];
	echo $_SESSION['Bussbiljett'];

	$sql = "INSERT INTO tillval_lop(kundnr, ordningsnr, startnr, diplom, forsakring, bussbiljett, langning) VALUES('$kundNr', '$lopp', '$startnr', '$diplom', '$forsakring', '$bussbiljett', '$langning')";
	$result = mysqli_query($dbc,$sql);
	// print_r($_POST);
	$_SESSION['startnr'] = $startnr;
	$_SESSION['angra_test']= 1;
...
// echo "<center><button id='angraknapp' input type='submit'><img src='pictures/pilVanster.jpg' width='50' height='30'></button>";
...
if(isset($_POST['angra'])){
	...
	$result = mysqli_query($dbc,$sql);
	echo "<script type='text/javascript'>  window.location='booking_lopning.php'; </script>";
}
