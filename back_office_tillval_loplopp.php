...
<br>
<h1 id="header"> Statistik för tillval löplopp</h1>
<br>
...
include ("connection.php");
	...
	if($usr != true){
		...
		echo '<option value="">Löplopp - Loppnamn - Distans - Klubbkrav</option>';
...
foreach ($dbc->query("SELECT artiklar_langning.namn, artiklar_langning.varde, COUNT(tillval_lop.langning) as total FROM artiklar_langning, tillval_lop WHERE tillval_lop.langning=artiklar_langning.id AND tillval_lop.ordningsnr = '$ordningsnr' GROUP BY artiklar_langning.id;") as $row) {
		echo "<tr>";
...
foreach ($dbc->query("SELECT artiklar_diplom.namn, artiklar_diplom.varde, COUNT(tillval_lop.diplom) as total from artiklar_diplom, tillval_lop WHERE tillval_lop.diplom=artiklar_diplom.id AND tillval_lop.ordningsnr = '$ordningsnr'  group by artiklar_diplom.id;") as $row) {
		echo "<tr>";
			echo"<td class='tabellinfo'>".$row['namn']."</td>";
			echo"<td class='tabellinfo'>".$row['varde']."</td>";
			echo"<td class='tabellinfo'>".$row['total']."</td>";
...
foreach ($dbc->query("SELECT artiklar_forsakring.namn, artiklar_forsakring.varde, COUNT(tillval_lop.forsakring) as total from artiklar_forsakring, tillval_lop where tillval_lop.forsakring=artiklar_forsakring.id AND tillval_lop.ordningsnr = '$ordningsnr' group by artiklar_forsakring.id;") as $row) {
		echo "<tr>";
			echo"<td class='tabellinfo'>".$row['namn']."</td>";
			echo"<td class='tabellinfo'>".$row['varde']."</td>";
			echo"<td class='tabellinfo'>".$row['total']."</td>";
...
foreach ($dbc->query("SELECT artiklar_biljett.namn, artiklar_biljett.varde, COUNT(tillval_lop.bussbiljett) as total from artiklar_biljett, tillval_lop where tillval_lop.bussbiljett=artiklar_biljett.id AND tillval_lop.ordningsnr = '$ordningsnr'  group by artiklar_biljett.id;") as $row) {
		echo "<tr>";
			echo "<td class='tabellinfo'>".$row['namn']."</td>";
			echo "<td class='tabellinfo'>".$row['varde']."</td>";
			echo "<td class='tabellinfo'>".$row['total']."</td>";
