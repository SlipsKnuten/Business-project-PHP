...
if(isset($_SESSION['login_user'])){
	...
	echo "<div id='wrapper'>";	
		...
		echo "<div id='tillval'>";
			...
			echo "<br><h1 id='våraLopp'>Våra löplopp</h1><br>";
			...
			echo "<center><form method='POST' id='bookingForm'>";
				...
				echo "<table id='loppform'>";
					echo "<tr>";
						...
						echo "<th>Välj</th><br>";
							foreach($dbc->query("SELECT * FROM loplopp") as $row){
								echo "<tr>";	
	...
	echo "</div>"; //wrapper
		...
		if(isset($_POST['ordnr'])){
			...
			$check = "SELECT * FROM loppanmalan_lop WHERE kundnr = '$kundnr' AND ordningsnr = '$ordnr'";
			$result = mysqli_query($dbc,$check);
			$count = mysqli_num_rows($result);
			//var_dump($choosenLopp);
			
				if($count == 0){
					$sth = $dbc->prepare("CALL lopseedning('$kundnr','$ordnr');");
					$sth->execute();
				
						if($loggedIn == true){
							header("location: tillval_lop.php");
							$ordnr = $_POST['ordnr'];
