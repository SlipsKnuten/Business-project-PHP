...
$loggedIn = $_SESSION['logged_in'];
	...
	echo "<div id='wrapper'>";
		...
		echo "<div id='column'>";
			...
			echo "<br><h1 id='våraLopp'>Våra Mountainbikelopp</h1><br>";
			...
			echo "<center><form method='POST' id='bookingForm'>";
				echo "<table id='loppform'>";
					echo "<tr>";
						...
						echo "<th>Välj</th><br>";
							foreach($dbc->query("SELECT * FROM mtblopp") as $row){
								echo "<tr>";	
	...
	echo "</div>"; //wrapper
		...
		if(isset($_POST['ordnr'])){
			...
			$check = "SELECT * FROM loppanmalan_mtb WHERE kundnr = '$kundnr' AND ordningsnr = '$ordnr'";
			$result = mysqli_query($dbc,$check);
			$count = mysqli_num_rows($result);
			// var_dump($choosenLopp);
			
			if($count == 0){
				$sth = $dbc->prepare("CALL mbtseedning('$kundnr','$ordnr');");
					$sth->execute();
				
				if($loggedIn == true){
					header("location: tillval_mtb.php");
					$ordnr = $_POST['ordnr'];
