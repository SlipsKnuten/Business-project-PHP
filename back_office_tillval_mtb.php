...
<br>
<h1 id="header"> Statistik för tillval MTB-lopp</h1>
...
include ("connection.php");
	...
	echo $_SESSION['logged_in'];
		...
		echo '<option value="">MTB - Loppnamn - Distans - Klubbkrav</option>';
		...
		//print_r($_POST);
				if(isset($_POST['ordningsnr'])){		//GÖR SÅ ATT INGET SKRIVS UT INNAN MAN GÖR SITT VAL
...
foreach ($dbc->query("SELECT artiklar_langning.namn, artiklar_langning.varde, COUNT(tillval_mtb.langning) as total FROM artiklar_langning, tillval_mtb WHERE tillval_mtb.langning=artiklar_langning.id AND tillval_mtb.ordningsnr = '$ordningsnr' GROUP BY artiklar_langning.id;") as $row) {
		echo "<tr>";
		...
		echo "</tr>";
			...
			<canvas id='diagram-langning';></canvas>
...
foreach ($dbc->query("SELECT artiklar_diplom.namn, artiklar_diplom.varde, COUNT(tillval_mtb.diplom) as total from artiklar_diplom, tillval_mtb WHERE tillval_mtb.diplom=artiklar_diplom.id AND tillval_mtb.ordningsnr = '$ordningsnr'  GROUP BY artiklar_diplom.id;") as $row) {
		echo "<tr>";
			echo "<td class='tabellinfo'>".$row['namn']."</td>";
			echo "<td class='tabellinfo'>".$row['varde']."</td>";
			echo "<td class='tabellinfo'>".$row['total']."</td>";
		...
		echo"</table>";
		?>
		<canvas id='diagram-diplom';></canvas>
		<?php
...
foreach ($dbc->query("SELECT artiklar_forsakring.namn, artiklar_forsakring.varde, COUNT(tillval_mtb.forsakring) as total from artiklar_forsakring, tillval_mtb WHERE tillval_mtb.forsakring=artiklar_forsakring.id AND tillval_mtb.ordningsnr = '$ordningsnr' GROUP BY artiklar_forsakring.id;") as $row) {
		echo "<tr>";
			echo "<td class='tabellinfo'>".$row['namn']."</td>";
			echo "<td class='tabellinfo'>".$row['varde']."</td>";
			echo "<td class='tabellinfo'>".$row['total']."</td>";
		...
		echo"</table>";
		?>
		<canvas id='diagram-forsakring';></canvas>
		<?php
...
foreach ($dbc->query("SELECT artiklar_biljett.namn, artiklar_biljett.varde, COUNT(tillval_mtb.bussbiljett) as total from artiklar_biljett, tillval_mtb WHERE tillval_mtb.bussbiljett=artiklar_biljett.id AND tillval_mtb.ordningsnr = '$ordningsnr'  GROUP BY artiklar_biljett.id;") as $row) {
		echo "<tr>";
			echo "<td class='tabellinfo'>".$row['namn']."</td>";
			echo "<td class='tabellinfo'>".$row['varde']."</td>";
			echo "<td class='tabellinfo'>".$row['total']."</td>";
		...
		<canvas id='diagram-biljett';></canvas>
<?php
echo "<br>";
echo "</div>";
...
?>
	...
	<script>
		var ctx = document.getElementById('diagram-langning').getContext('2d');
		...
		<script>
		var ctx = document.getElementById('diagram-diplom').getContext('2d');
		var chart = new Chart(ctx, {
		// The type of chart we want to create
		type: 'bar',
		...
		<script>
		var ctx = document.getElementById('diagram-forsakring').getContext('2d');
		var chart = new Chart(ctx, {
		// The type of chart we want to create
		type: 'bar',
		...
		<script>
		var ctx = document.getElementById('diagram-biljett').getContext('2d');
		var chart = new Chart(ctx, {
		// The type of chart we want to create
		type: 'bar',
