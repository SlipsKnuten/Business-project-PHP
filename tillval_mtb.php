			...
			echo "<br>";
				...
				echo "<tr class='tillvalLop1'>";
					...
					foreach($dbc->query("SELECT * FROM artiklar_langning")as $row) {
						...
						$x++;
						$L = 'l' . $x;
						echo "<td class='beskrivning'><p class='items'>".$row['beskrivning']."</p></td>";
						}
					echo "</tr>";
...
if(isset($_POST['langning'])){
	...
	$sql = "SELECT pris FROM mtblopp WHERE ordningsnr = '$ordningsnr'";
	$result = mysqli_query($dbc, $sql);
	...
	$sql = "INSERT INTO tillval_mtb(kundnr, startnr, ordningsnr, diplom, forsakring, bussbiljett, langning) VALUES('$kundNr', '$startnr', '$lopp', '$diplom', '$forsakring', '$bussbiljett', '$langning')";
	$result = mysqli_query($dbc,$sql);
	echo $kundNr;
	echo $lopp;
	echo $startnr;
...
if(isset($_POST['angra'])){
	...
	$sql = "DELETE FROM loppanmalan_mtb WHERE startnr = '$startnr'";
	$result = mysqli_query($dbc,$sql);
	echo "<script type='text/javascript'>  window.location='booking_mtb.php'; </script>";
}
?>
	</div>
	</center>
