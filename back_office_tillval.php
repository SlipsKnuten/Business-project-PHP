...
<h1 id="header"> Statistik för tillval skidlopp</h1>
<br>
...
foreach ($dbc->query("SELECT artiklar_valla.namn, artiklar_valla.varde, COUNT(tillval_skidor.valla) as total from artiklar_valla, tillval_skidor where tillval_skidor.valla=artiklar_valla.id group by artiklar_valla.id;") as $row) {
		echo "<tr>";
...
foreach ($dbc->query("SELECT artiklar_langning.namn, artiklar_langning.varde, COUNT(tillval_skidor.langning) as total from artiklar_langning, tillval_skidor where tillval_skidor.langning=artiklar_langning.id group by artiklar_langning.id;") as $row) {
		echo "<tr>";
			echo "<td class='tabellinfo'>".$row['namn']."</td>";
			echo "<td class='tabellinfo'>".$row['varde']."</td>";
			echo "<td class='tabellinfo'>".$row['total']."</td>";
...
foreach ($dbc->query("SELECT artiklar_diplom.namn, artiklar_diplom.varde, COUNT(tillval_skidor.diplom) as total from artiklar_diplom, tillval_skidor where tillval_skidor.diplom=artiklar_diplom.id group by artiklar_diplom.id;") as $row) {
		echo "<tr>";
			echo "<td class='tabellinfo'>".$row['namn']."</td>";
			echo "<td class='tabellinfo'>".$row['varde']."</td>";
			echo "<td class='tabellinfo'>".$row['total']."</td>";
...
foreach ($dbc->query("SELECT artiklar_forsakring.namn, artiklar_forsakring.varde, COUNT(tillval_skidor.forsakring) as total from artiklar_forsakring, tillval_skidor where tillval_skidor.forsakring=artiklar_forsakring.id group by artiklar_forsakring.id;") as $row) {
		echo "<tr>";
			echo "<td class='tabellinfo'>".$row['namn']."</td>";
			echo "<td class='tabellinfo'>".$row['varde']."</td>";
			echo "<td class='tabellinfo'>".$row['total']."</td>";
...
foreach ($dbc->query("SELECT artiklar_biljett.namn, artiklar_biljett.varde, COUNT(tillval_skidor.bussbiljett) as total from artiklar_biljett, tillval_skidor where tillval_skidor.bussbiljett=artiklar_biljett.id group by artiklar_biljett.id;") as $row) {
		echo "<tr>";
			echo "<td class='tabellinfo'>".$row['namn']."</td>";
			echo "<td class='tabellinfo'>".$row['varde']."</td>";
			echo "<td class='tabellinfo'>".$row['total']."</td>";
...
<script>
        ...
        datasets: [{
            labels: ["Ingen Valla", "Glidvallning, LF", "Glidvallning HF", "Vallning LF", "Vallning HF", "värsting"],
            backgroundColor: ["red", "blue", "green", "orange", "brown", "yellow"],
            borderColor: '#587092',
...
<script>
        ...
        datasets: [{
            label: ["Sålda lagningspaket paket", "hej", "hopp"],
